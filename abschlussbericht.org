...
der Schwerpunkt hier nur auf zwei Aspekten, nämlich dem Datenschutz
...
Mitglieder, mit denen es sich verbunden hat. Da Kademlia ein sehr
umfangreiches, kompliziertes Protokoll und System beschreibt, werden
hier nur einige zentrale Funktionen besprochen, die für diesen ersten
Prototypen von Project Orion relevant sind.\\
...
Frage beantworten, in wie vielen Unterbäumen Verbindungen gehalten
...
lassen sich für die neuen Zielsetzung vollständig übernehmen. Die
...
einzelne Anwendung angestrebt wurde, ging es stattdessen um die
...
nicht richtig isoliert und verarbeitet würden, weswegen sich die
Probleme durch das System weiter verbreiten würden. Während die
...
- Orion beschreibt das Videospiel, welches als zentrale Demo dem
...
hängen alle Programme von Actaeon ab. Um diese Integration einfacher
zu machen, sind nahezu alle Applikationen in Project Orion in der
...
den folgenden Abschnitten werden dann zusätzlich noch technische
Details zur Bibliothek angesprochen, diese sind aber nicht notwendig,
um die Bibliothek zu verwenden. Für die technischen Feinheiten lohnt
es sich, die online Dokumentation und Package Informationen zu lesen,
welche auf [[https:crates.io/crates/actaeon][crates.io]] verfügbar sind.
...
potentiell aufkommende Fehler zu handhaben. Das zweite wichtige
...
der Daten empfangen werden, oder die durch eine Anfrage gefunden
...
- Grundsätzlich werden alte, bereits seit langem bekannte Mitglieder
  gegenüber neuere bevorzugt, selbst wenn diese ansonsten bessere
  Eigenschaften (kleine Distanz) hätten. Damit wird verhindert, dass
  das Netzwerk durch eine Vielzahl böswillig generierter Nodes
...
ein k-Bucket dar, in welchem die tatsächlichen Mitglieder liegen.
Sobald ein solcher Bucket voll ist, kann er halbiert werden und es
entstehen zwei neue Blätter. Dies ist allerdings nur auf der dem
Zentrum (also der eigenen Adresse) nahen Seite möglich. Auf der /fernen/
Seite können Buckets nicht geteilt werden. Damit wird die Regel, dass
die Häufigkeit der Mitglieder mit steigendem Abstand abnimmt
umgesetzt. Sollte es nicht möglich sein, einen Bucket zu teilen,
werden neue Mitglieder entweder vergessen, oder bereits vorhandene
Mitglieder werden überschrieben. Auch ist es wichtig zu verstehen,
dass innerhalb der tatsächlichen Buckets nicht nach Distanz, sondern
nach Zeit sortiert wird. Da Nachrichten meistens an mehrere Ziele
gleichzeitig geschickt werden, um die Auslieferung zu garantieren,
wird versucht, zuerst möglichst viele Mitglieder aus dem gleichen
Bucket zu verwenden. Nur wenn diese nicht ausreichen werden Mitglieder
aus anderen Buckets verwendet.
...
\noindent TODO: Screenshots & Bilder + Kurze Erklärung. (Spieler,
Welt, UI)
