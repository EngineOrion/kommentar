...
* Einleitung 
Am 29. Oktober 1964 wurde von der Universität Kaliforniens folgende
Nachricht an das 500 Kilometer entfernte Stanford geschickt: =LO=.\\
...
einfachste Lösung scheint jedes mal immer stärkere Zentralisierung.
Doch damit kommen nebst den ideologischen Bedenken auch technische
Probleme, für welche wiederum neue Lösungen gefunden werden müssen.
...
sich tatsächlich noch unternehmen, ohne dabei an massive Unternehmen,
...
Idee ist dabei immer die selbe:\\
...
Personen geniessen. Allerdings gehen die Einsatzmöglichkeiten für
dezentrale Datensysteme weit über =file sharing= hinaus. Wer aber von
den fortgeschritteneren Funktionen dezentraler Systeme profitieren
will, muss meistens mit mehr Aufwand rechnen und viel Know-How
mitbringen. 
* Zielsetzung
Die Vorteile dezentraler Systeme sind vielseitig, allerdings hat das
Gebiet, aus offensichtlichen Gründen, wenig Interesse von grossen
Firmen oder Initiativen erhalten. Nebst der offensichtlichen
Fehlertoleranz, welche die Dezentralisierung mit sich bringt,
öffnen dezentrale Systeme eine neue Welt der Möglichkeiten:
- Moderne Videospiele haben meist starke Limitierungen, wenn es um die
  Anzahl Spieler auf einmal geht. Diese existieren allerdings
  hauptsächlich nur, weil die zentralen Server, welche die
  Verbindungen zwischen den Spielern möglich machen, nicht mehr
  Nutzer verarbeiten können. Dezentrale Systeme könnten diese Limits
  nahezu komplett aufheben.
- Bei den meisten sozialen Interaktionen zwischen Personen über digitale
  Plattformen geht es eigentlich um den Kontakt. Die Plattform agiert
  dabei nur als Vermittler und ist nicht zwingend für die Aktivität.
  Damit wären auch beispielsweise soziale Netzwerke oder Chat-Dienste
  offen für dezentrale Alternativen offen.
...
- Auch wenn oftmals digitale Dienste als Methode zum Datenaustausch
...
Anhang genauer angeschaut. Welche Lücke kann Project Orion füllen?
- Da das Entwickeln einer vollständigen, getesteten und integrierten
  Applikation im gegebenen Zeitrahmen nicht möglich ist, sollen
  ...
  ähnliche, sondern um Prototypen, die die technischen Möglichkeiten
  aufzeigen sollen.
...
- Als Anwendungen sollen die folgenden drei Systeme ein möglichst
  grosse Auswahl der möglichen Applikationen ab:
  ...
  - Mit einer eigenen, spezialisierten Programmiersprache sollen die
    ...
    Automatisierung gezeigt werden. Dazu soll die Sprache allgemein in
    das dezentrale Netzwerke eingebunden werden und soll somit auch
    direkt mit Nutzern kommunizieren können.
  - Als zentrale Demo soll mit einem Videospiel gezeigt werden, mit
    wie wenig Aufwand dezentrale Technologien sichtbare Vorteile in
    Videospielen und anderen Echtzeit-Applikationen machen können.
* Theorie
...
Grundlagen erklärt werden. Dabei geht es nicht nur um dezentrale
Systeme gehen, sondern es sollen auch detaillierter einige der bereits
erwähnten Probleme angesprochen werden. Dafür müssen einige der
technischen Details des aktuellen Internets angesprochen werden. Dabei
liegt ein Schwerpunkt auf dem Adress- und Routing-System, welche nicht
nur viele Probleme aufweisen, sondern auch gute Alternativen möglich
sind.
** Adressen
...
Betroffenen bereit sein müssen, sondern für die auch jede einzelne
involvierte Komponente diese neue Technologie unterstützen muss.
...
sondern u Abschnitte von Adressen, auch =Address Spaces= genannt.
[[cite:&IPAddressSpaces]] An den zentralen Knotenpunkten geht es also
nicht um einzelne Server oder Geräte, zu dem etwas gesendet werden
muss, stattdessen wird eher entschieden, ob gewisse Daten
beispielsweise von Frankfurt a. M. aus nach Ost- oder Westeuropa
...
dieser Abschnitt unendlich in die Länge gezogen werden, allerdings
soll der Schwerpunkt hier nur auf zwei Aspekten liegen, nämlich dem
...
mehr anpassen könnten [[cite:&GoogleNestOutage]], das Netz aus internen
...
wirtschaftliche Schaden solcher Ausfälle ist unvorstellbar, aber noch
wichtiger muss die zerstörende Wirkung dieser unvorhergesehenen,
scheinbar entfernten Problemen auf Millionen von Menschen bedacht
werden.
** Kademlia
...
beschrieben, welches seither in verschiedensten Gebieten Nutzen
...
\noindent Anstelle des /Frontends/ soll hier direkt der Code angeschaut
werden, welcher diese Demo möglich macht:
#+begin_src rust
  use actaeon::{
      config::Config,
      node::{Center, ToAddress},
...
#+end_src
\noindent Eine kurze, oberflächliche Erklärung was mit diesem Code
erreicht wird:
